...
const RC_FILENAME = ".localmodulesrc";
...
class PackagePaths {
    constructor( packagePath, options ) {
        ...
        this.rc = path.resolve(this.modulePath, RC_FILENAME);
     ...
     * @returns
     */
    resolveTarBallName(json) {
        var baseName = json.name.replace(/@/gi,'').replace(/\//gi, '-');
        return `${baseName}-${json.version}.tgz`;
    }
}
