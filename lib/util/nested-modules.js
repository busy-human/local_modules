...
/**
 * Runs the pack command on local modules contained in local modules
 * AND copies/installs the tar file to this project
 * @param {*} moduleDirectory
 */
function packNestedModules(options, moduleDirectory) {
    if(moduleContainsLocalModules(moduleDirectory)) {
        exec(`cd "${moduleDirectory}"; lm pack`);
        copyNestedTarFiles(options, moduleDirectory);
    }
}
 ...
 */
function copyNestedTarFiles(options, moduleDirectory) {
    const local_modules = options.pkg.json.local_modules;
    var keys = Object.keys(local_modules.packed);
    keys.forEach(k => {
        var tarPath = local_modules.packed[k];
        exec(`cd "${moduleDirectory}"; cp "./${tarPath}" "${options.dirPath}"`)
    });
}