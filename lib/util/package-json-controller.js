...
class PackageJsonController {
    ...
    removeTemporaryModules() {
        this.forEachTempPacked((name) => {
            delete this.dependencies[name];
        });
        this.local_modules.tempPacked = {};
        this.write();
    }

    forgetPackedRecords() {
        this.local_modules.packed = {};
        this.write();
    }
    ...
    forEachTempPacked( fn ) {
        var keys = Object.keys(this.local_modules.tempPacked);
        keys.forEach(key => {
            fn(key, this.local_modules.tempPacked[key]);
        });
    }
